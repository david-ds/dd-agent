...
class WMISampler(Thread):
    ...
    def __del__(self):
        self.logger.debug("destructor %s" %self.class_name)
        self.stop()
        self.logger.debug("destructor done %s" %self.class_name)
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                self.logger.info("results len for %s is %d" % (self.class_name, len(results)))
            ...
            self.logger.debug("wmi query run complete")
            self.firstPassEvent.set();
            self.logger.debug("run started %s" % str(start))
            now = time.time()
            self.logger.debug("run completed %s" %str(now))

            last_duration = now - start
            self.logger.debug("duration was %s" % str(last_duration))
            if last_duration < 15:
                waittime = 15 - last_duration
            else:
                waittime = 0
            self.logger.debug("sampler thread waiting %d" % waittime)
    ...
    def stop(self):
        self.logger.info("sampler.stop() %s" % self.class_name)
        self.stopEvent.set()
        self.join()
    ...
    def __iter__(self):
        ...
        if self.is_raw_perf_class:
            ...
            for previous_wmi_object, current_wmi_object in \
                ...
                self.logger.debug("pvs %s"% str(previous_wmi_object))
                self.logger.debug("cur %s"% str(current_wmi_object))
